...
| **流畅动效 / 微交互** | chip / 齿轮 / hub 面板 / modal / toast 全套入场动画 + hover 微交互，统一缓动；尊重 `prefers-reduced-motion` |
| **智能 codex 接入向导** | 新服务器零命令行接入：网页自动识别 codex 路径 + `device-auth` 扫码登录 + 自动测试 + 健康诊断 |
